...
def merge_discipline_jsons_to_jsonl(root_dir: str) -> None:
    ...
    for discipline in os.listdir(root_dir):
        ...
        try:
            ...
            languages = [
                d
                for d in os.listdir(discipline_dir)
                if os.path.isdir(os.path.join(discipline_dir, d))
            ]

            with gzip.open(output_file, "wt", encoding="utf-8") as outfile:
                for lang in sorted(languages):
                    lang_dir = os.path.join(discipline_dir, lang)

                    # Get all year directories for this language
                    years = sorted(
                        [
                            d
                            for d in os.listdir(lang_dir)
                            if os.path.isdir(os.path.join(lang_dir, d)) and d.isdigit()
                        ]
                    )
                    ...
                    for year in years:
                        ...
                        final_dir_paths = [
                            os.path.join(year_dir, _)
                            for _ in os.listdir(year_dir)
                            if os.path.isdir(os.path.join(year_dir, _))
                        ]

                        for d_path in sorted(final_dir_paths):

                            datestamp_dir = [
                                os.path.join(d_path, _)
                                for _ in os.listdir(d_path)
                                if os.path.isdir(os.path.join(d_path, _))
                            ]

                            for d_path in sorted(datestamp_dir):
                                try:
                                    ...
                                    logging.info(
                                        f"Processing directory: {d_path} with files: {len(json_files)}"
                                    )

                                    # process each JSON file
                                    for file_index in tqdm(range(len(json_files))):
                                        json_file = json_files[file_index]
                                        file_path = os.path.join(d_path, json_file)
                                    ...
                                    logging.error(
                                        f"Error processing year {year} and datestamp {d_path}: {str(e)}"
                                    )

            logging.info(
                f"Successfully created gzipped JSONLines file for {discipline}: {output_file}"
            )

        except Exception as e:
            logging.error(f"Error processing discipline {discipline}: {str(e)}")
